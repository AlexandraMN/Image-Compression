function cerinta2()
	...
	A1 = imread(image1);
	A1 = double(A1);
	% Pentru cerinta 2.1 am aflat cu svd matricea S care contine 
	% valorile singulare in ordine descrescatoare si le-am pus intr-un vector
	...
	subplot(2, 4, 3);
	plot(1 : min1, k2);
	title('Imaginea1, cerinta2.3')

	% Cerinta 2.4
	% Am allat rata de compresie a datelor 	
	k3 = zeros(min1, 1);
	for i = 1 : min1
		k3(i, 1) = (m1 * i + n1 * i + i) / (m1 * n1);
	endfor
	subplot(2, 4, 4);
	plot(1 : min1, k3);
	title('Imaginea1, cerinta2.4')
	...
	subplot(2, 4, 7);
	plot(1 : min2, w2);
	title('Imaginea2, cerinta2.3')

	w3 = zeros(min2, 1);
	for i = 1 : min2
		w3(i, 1) = (m2 * i + n2 * i + i) / (m2 * n2);
	endfor
	subplot(2, 4, 8);
	plot(1 : min2, w3);
	title('Imaginea2, cerinta2.4')
